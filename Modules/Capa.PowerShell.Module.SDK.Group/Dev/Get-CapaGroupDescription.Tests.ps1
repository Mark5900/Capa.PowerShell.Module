BeforeAll {
	...
	$script:GroupName = "TestGroupDesc_$([DateTime]::Now.ToString('yyyyMMddHHmmss'))"
...
Describe 'Get-CapaGroupDescription' {
	It 'Returns the description string for an existing group' {
		if (-not $script:CreatedGroup) {
			Set-ItResult -Skipped -Because 'Insufficient rights to create groups in this environment.'
			return
		}

		$Result = Get-CapaGroupDescription -CapaSDK $oCMS -GroupName $script:GroupName -GroupType Static
		$Result | Should -Not -Be $null
	}

	It 'Throws when GroupName is empty' {
		{ Get-CapaGroupDescription -CapaSDK $oCMS -GroupName '' -GroupType Static } | Should -Throw
	}
}
