BeforeAll {
	...
	$script:GroupName = "TestGroupPkgs_$([DateTime]::Now.ToString('yyyyMMddHHmmss'))"
...
Describe 'Get-CapaGroupPackages' {
	It 'Returns an array (possibly empty) of packages for a group' {
		if (-not $script:CreatedGroup) {
			Set-ItResult -Skipped -Because 'Insufficient rights to create groups in this environment.'
			return
		}

		$Result = Get-CapaGroupPackages -CapaSDK $oCMS -GroupName $script:GroupName -GroupType Static
		$Result | Should -Not -Be $null
	}

	It 'Throws when GroupName is empty' {
		{ Get-CapaGroupPackages -CapaSDK $oCMS -GroupName '' -GroupType Static } | Should -Throw
	}
}
