BeforeAll {
	...
	$script:PackageName = "TestSetUnitPackageStatus_$([DateTime]::Now.ToString('yyyyMMddHHmmss'))"
...
Describe 'Set-CapaUnitPackageStatus integration' {
	It 'Can set package status on an existing unit for a real package' {
		...
		$SetStatus | Should -Be $true

		$ObservedStatus = $null
		for ($i = 0; $i -lt 10; $i++) {
			Start-Sleep -Seconds 2
			$ObservedStatus = Get-CapaUnitPackageStatus -CapaSDK $oCMSProd -UnitName $script:UnitName -UnitType 'Computer' -PackageName $script:PackageName -PackageVersion $script:PackageVersion
