BeforeAll {
	...
	Import-Module "$RootPath\Capa.PowerShell.Module.PowerPack.Exit\Dev\Exit-PpScript.ps1" -Force
...
Describe '$global:gsComputerName' {
	It "Should be $env:COMPUTERNAME" {
		$global:gsComputerName | Should -Be $env:COMPUTERNAME
	}
	It 'Should be a string' {
		$global:gsComputerName | Should -BeOfType [string]
	}
	It 'Should be the same as $Global:Cs' {
		$global:gsComputerName | Should -Be $Global:Cs.gsComputerName
	}
}
