BeforeAll {
	...
	$script:GroupName = "TestSetGroupDesc_$([DateTime]::Now.ToString('yyyyMMddHHmmss'))"
...
Describe 'Set-CapaGroupDescription' {
	It 'Sets the description on a group' {
		if (-not $script:CreatedGroup) {
			Set-ItResult -Skipped -Because 'Insufficient rights to create groups in this environment.'
			return
		}

		$Result = Set-CapaGroupDescription -CapaSDK $oCMS -GroupName $script:GroupName -GroupType Static -Description 'Test description' -Confirm:$false
		$Result | Should -Not -BeNullOrEmpty
	}

	It 'Throws when GroupName is empty' {
		{ Set-CapaGroupDescription -CapaSDK $oCMS -GroupName '' -GroupType Static -Confirm:$false } | Should -Throw
	}
}
