BeforeAll {
	...
	if ($null -eq $Units -or $Units.Count -eq 0) {
		throw 'No computer units found. Cannot run Get-CapaLogonHistoryForUnit integration test.'
	}
...
Describe 'Get-CapaLogonHistoryForUnit' {
	It 'Returns logon history for a computer unit' {
		$Result = Get-CapaLogonHistoryForUnit -CapaSDK $oCMS -UnitName $script:TargetUnit.Name -UnitType Computer
		$Result | Should -Not -Be $null
	}

	It 'Throws when UnitName is empty' {
		{ Get-CapaLogonHistoryForUnit -CapaSDK $oCMS -UnitName '' -UnitType Computer } | Should -Throw
	}
}
