BeforeAll {
	...
	if ($null -eq $Units -or $Units.Count -eq 0) {
		throw 'No computer units found. Cannot run Get-CapaSoftwareInventoryForUnit integration test.'
	}
...
Describe 'Get-CapaSoftwareInventoryForUnit' {
	It 'Returns software inventory by unit name and type' {
		$Result = Get-CapaSoftwareInventoryForUnit -CapaSDK $oCMS -UnitName $script:TargetUnit.Name -UnitType Computer
		$Result | Should -Not -Be $null
	}

	It 'Throws when UnitName is empty' {
		{ Get-CapaSoftwareInventoryForUnit -CapaSDK $oCMS -UnitName '' -UnitType Computer } | Should -Throw
	}
}
