...
```
Get-CCSADSid [-ComputerName] <string[]> -Domain <string> -Url <string> -CCSCredential <pscredential>
 [-DomainOUPath <string>] [-DomainCredential <pscredential>] [-PasswordIsEncrypted]
 [<CommonParameters>]
```
...
Get-CCSADSid -ComputerName "TestPC" -Domain "example.com" -Url "https://example.com/CCSWebservice/CCS.asmx" -CCSCredential $Credential

Retrieves the SID for TestPC using default CCS context.

### EXAMPLE 2

Get-CCSADSid -ComputerName "TestPC" -DomainOUPath "OU=Computers,DC=example,DC=com" -Domain "example.com" -Url "https://example.com/CCSWebservice/CCS.asmx" -CCSCredential $Credential -DomainCredential $DomainCredential

Retrieves the SID for TestPC using specific domain credentials and OU path.

### EXAMPLE 3

"PC01", "PC02", "PC03" | Get-CCSADSid -Domain "example.com" -Url "https://example.com/CCSWebservice/CCS.asmx" -CCSCredential $Credential

Retrieves SIDs for multiple computers using pipeline input.

### EXAMPLE 4

Get-CCSADSid -ComputerName "TestPC" -DomainOUPath "LDAP://DC01.Firmax.local/DC=FirmaX,DC=local" -Domain "Firmax.local" -Url "https://example.com/CCSWebservice/CCS.asmx" -CCSCredential $Credential

Retrieves the SID using LDAP format for the OU path.
