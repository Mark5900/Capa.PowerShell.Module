BeforeAll {
	...
	if ($null -eq $Units -or $Units.Count -eq 0) {
		throw 'No computer units found. Cannot run Get-CapaUpdateInventoryForUnit integration test.'
	}
...
Describe 'Get-CapaUpdateInventoryForUnit' {
	It 'Returns update inventory for a computer unit' {
		$Result = Get-CapaUpdateInventoryForUnit -CapaSDK $oCMS -UnitName $script:TargetUnit.Name -UnitType Computer
		$Result | Should -Not -Be $null
	}

	It 'Throws when UnitName is empty' {
		{ Get-CapaUpdateInventoryForUnit -CapaSDK $oCMS -UnitName '' -UnitType Computer } | Should -Throw
	}
}
