BeforeAll {
	...
	if ($null -eq $Units -or $Units.Count -eq 0) {
		throw 'No computer units found. Cannot run Get-CapaCustomInventoryForUnit integration test.'
	}
...
Describe 'Get-CapaCustomInventoryForUnit' {
	It 'Returns custom inventory by unit name and type' {
		$Result = Get-CapaCustomInventoryForUnit -CapaSDK $oCMS -UnitName $script:TargetUnit.Name -UnitType Computer
		$Result | Should -Not -Be $null
	}

	It 'Throws when UnitName is empty' {
		{ Get-CapaCustomInventoryForUnit -CapaSDK $oCMS -UnitName '' -UnitType Computer } | Should -Throw
	}
}
