BeforeAll {
	...
	$script:GroupName = "TestGroupFolder_$([DateTime]::Now.ToString('yyyyMMddHHmmss'))"
...
Describe 'Get-CapaGroupFolder' {
	It 'Returns folder path for an existing group' {
		if (-not $script:CreatedGroup) {
			Set-ItResult -Skipped -Because 'Insufficient rights to create groups in this environment.'
			return
		}

		$Result = Get-CapaGroupFolder -CapaSDK $oCMS -GroupName $script:GroupName -GroupType Static
		$Result | Should -Not -Be $null
	}

	It 'Throws when GroupName is empty' {
		{ Get-CapaGroupFolder -CapaSDK $oCMS -GroupName '' -GroupType Static } | Should -Throw
	}
}
